...
/// Renders `req` with `add_generation_prompt` forced to a given value.
///
/// MiniJinja's `context!{ ..a, ..b }` resolves `a` first, so `chat_template_args` can add
/// keys but cannot override the flag the renderer already put in the context.
...
impl OpenAIPreprocessor {
    ...
    /// Decode token ids through this model's tokenizer.
    pub fn detokenize(
        &self,
        token_ids: &[TokenIdType],
